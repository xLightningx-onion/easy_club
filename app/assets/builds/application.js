...
var WARNING_CLASSES = ["border-rose-500", "focus:border-rose-500", "focus:ring-rose-200"];
var email_check_controller_default = class extends Controller {
  ...
  queueCheck() {
    ...
    this.timeoutId = setTimeout(() => this.performLookup(email), 1e3);
  }
  async performLookup(email) {
    ...
    try {
      ...
      if (!response2.ok) throw new Error("Request failed");
...
var WARNING_CLASSES2 = ["border-rose-500", "focus:border-rose-500", "focus:ring-rose-200"];
var mobile_check_controller_default = class extends Controller {
  ...
  queueCheck() {
    ...
    this.timeoutId = setTimeout(() => this.performLookup(countryCode, mobile, cacheKey), 1e3);
  }
  ...
  showWarning() {
    if (this.hasInputTarget) {
      this.inputTarget.setAttribute("aria-invalid", "true");
      WARNING_CLASSES2.forEach((className) => this.inputTarget.classList.add(className));
    }
    if (this.hasMessageTarget) {
      const loginUrl = this.loginUrlValue || "#";
  ...
  clearWarning() {
    if (this.hasInputTarget) {
      this.inputTarget.removeAttribute("aria-invalid");
      WARNING_CLASSES2.forEach((className) => this.inputTarget.classList.remove(className));
    }
    if (this.hasMessageTarget) {
      this.messageTarget.classList.add("hidden");
      this.messageTarget.textContent = "";
...
// app/javascript/controllers/login_method_controller.js
var login_method_controller_default = class extends Controller {
  static targets = [
    "mobileContainer",
    "mobileInput",
    "emailContainer",
...
application.register("email-check", email_check_controller_default);
application.register("mobile-check", mobile_check_controller_default);
application.register("login-method", login_method_controller_default);
