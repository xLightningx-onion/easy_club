...
class Club::CheckoutsController < Club::BaseController
  def create
    ...
    if payment_source == "saved"
      ...
      if payment_method.nil?
        redirect_to club_cart_path, alert: "Please select a saved card or enter a new card." and return
      end

      saved_card_cvv = normalize_saved_card_cvv(checkout[:saved_card_cvv])
      if saved_card_cvv.blank?
        redirect_to club_cart_path, alert: "Please enter the CVV for your saved card." and return
      end
    else
      payment_method = nil
    end
    ...
    if result.order.status_paid?
      redirect_to club_checkout_success_path(reference: result.order.number)
    else
  ...
  def success
    ...
    if reference.blank?
      redirect_to club_cart_path, alert: "We couldn't find that payment confirmation."
      return
    end
