...
class Club::CartItemsController < Club::BaseController
  ...
  def create
    ...
    if @cart_item.save
      redirect_to club_cart_path, notice: "Membership added to cart."
    else
      redirect_back fallback_location: club_cart_path, alert: @cart_item.errors.full_messages.to_sentence
    end
  end
  ...
  def update
    ...
    if quantity <= 0
      @cart_item.destroy
      redirect_to club_cart_path, notice: "Item removed from cart."
    elsif @cart_item.update(quantity: [quantity, 1].max)
      redirect_to club_cart_path, notice: "Cart updated."
    else
      redirect_back fallback_location: club_cart_path, alert: @cart_item.errors.full_messages.to_sentence
    end
  end
